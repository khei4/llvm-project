...
bool MemCpyOptPass::performStackMoveOptzn(Instruction *Load, Instruction *Store,
  ...
  if (!LifetimeMarkers.empty()) {
    ...
    auto *FirstMA = MSSA->getMemoryAccess(Dom);
    auto *StartMA = MSSAU->createMemoryAccessBefore(Start, FirstMA->getDefiningAccess(), FirstMA);
    ...
    // MemoryAccess to consistent MemorySSA.
    // FIXME: Currently, only insert lifetime.end if the post-dominator is
    // the user of SrcAlloca, but even when it's not, we can probably insert
    // MemoryPhi.
    Instruction *PDomUser =
    ...
    if (PDom && PDomUser) {
      ...
      auto *LastMA = MSSA->getMemoryAccess(PDomUser);
